...
- create some kind of prompt matrix in ".\data\prompt_matrix.py", optimally coded, so as for all common required logical prompts are produced to the correctly relating models, in order to carry out common tasks, this may include using the tools, or processing responses/output from tools. you must be aware of the current tools, in order for tasks to be able to be accomplished. This will require deep analysis of currently implemented functions, and their relations, so as, to come up with the optimal configuration and operation. Functions in ".\scripts\model_interact.py" will require resulting upgrade.
